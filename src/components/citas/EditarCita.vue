                ...
                <form @submit.prevent="updateCita">
                    <div class="row mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" class="form-control" id="id" placeholder="ID de cita" disabled 
                            v-model="cita.id">
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        ...
        async updateCita() {
            try {
                const res = await axios.put(`http://localhost:8000/api/citas/${this.cita.id}`, this.cita);

                if (res.status === 200) { 
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
                        title: 'Cita ha sido actualizada correctamente',
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'No se pudo actualizar la cita. Por favor, intente de nuevo.',
                });
            }
        }
    },
    mounted(){
        const citaId = this.$route.params.id;

        axios.get(`http://localhost:8000/api/citas/${citaId}`)
            .then(response => {
